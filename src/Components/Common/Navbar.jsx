    ...
    <>
      <header className="w-full bg-white relative z-50">
        <Container className="relative">
          ...
          <div className="flex items-center justify-between w-full py-4 lg:py-6 relative z-10 gap-4">
            {/* Logo Column */}
            <div className="flex-1 flex justify-start items-center shrink-0">
              <img
                src="/befirst.svg"
                alt="beFirst Enterprises"
                className="h-10 md:h-12 lg:h-16 xl:h-20 2xl:h-28 w-auto object-contain"
              />
            </div>

            {/* Desktop Nav Column */}
            <div className="hidden lg:flex flex-[3] justify-center items-center">
              <nav className="bg-[#0d0d2a] rounded-full px-4 lg:px-6 xl:px-8 2xl:px-12 py-3 xl:py-4 2xl:py-5 flex items-center space-x-3 lg:space-x-5 xl:space-x-8 2xl:space-x-12 shadow-lg border border-white/5">
                {navLinks.map((link, index) => (
                  <a
                    key={index}
                    href={link.href}
                    onClick={(e) => { e.preventDefault(); handleNavClick(link.href); }}
                    className="text-[11px] lg:text-[13px] xl:text-sm 2xl:text-base font-medium transition-colors whitespace-nowrap text-white/80 hover:text-white hover:border-b-2 hover:border-[#c49c5e] pb-1"
                  >
                    {link.name}
                  </a>
                ))}
            ...
            {/* Right Column (Hamburger / Spacer) */}
            <div className="flex-1 flex justify-end items-center">
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="text-[#0d0d2a] focus:outline-none p-2 lg:hidden z-[100]"
              ...
              {/* Desktop spacer to maintain balance */}
